...
router.get('/', protect, verifyCAClientAssociation, async (req, res) => {
  try {
    ...
    const records = await ROCRecord.find(query).populate('client', 'name').sort({ createdAt: -1 });
    res.json(records);
  } catch (err) {
...
router.post('/', protect, requireCA, verifyCAClientAssociation, async (req, res) => {
  try {
    const record = await ROCRecord.create(req.body);
...
router.put('/:id', protect, requireCA, verifyCAClientAssociation, async (req, res) => {
  try {
    const record = await ROCRecord.findByIdAndUpdate(req.params.id, req.body, { new: true });
    res.json(record);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
